...
const PLACEMENT_SELECT = `
  SELECT p.id AS placement_id, a.id AS advertiser_id, a.mode, a.canonical_key,
         ...
         p.total_cents, p.click_count, p.reached_current_total_at
    FROM placements p
...
export async function listPlacements(territoryId: number): Promise<Row[]> {
  return normalizeRows(await q(PLACEMENT_SELECT, [territoryId]))
}

/** Aynı sorgunun transaction içinde çalışan hâli. */
async function listPlacementsTx(c: PoolClient, territoryId: number): Promise<Row[]> {
  const r = await c.query(PLACEMENT_SELECT, [territoryId])
    ...
    const credit = async (terrId: number, bundled: boolean) => {
      const pl = await one<{ id: number; total_cents: number }>(
        ...
        `SELECT id, total_cents FROM placements
          WHERE territory_id = $1 AND advertiser_id = $2 FOR UPDATE`,
        [terrId, advId],
      )
      ...
      if (pl) {
        ...
        const ins = await one<{ id: number }>(
          c,
          `INSERT INTO placements (territory_id, advertiser_id, total_cents)
           VALUES ($1,$2,$3) RETURNING id`,
          [terrId, advId, newTotal],
        )
      ...
      const rows = await listPlacementsTx(c, terrId)
      const rank = rows.findIndex((r) => r.advertiser_id === advId) + 1

      await c.query(
        `INSERT INTO activity (public_id, territory_id, advertiser_id, amount_cents, rank_after)
         VALUES ($1,$2,$3,$4,$5)`,
        [randomUUID(), terrId, advId, amount, rank],
      )
      return rank
    }
